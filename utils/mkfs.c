...
//    uint32_t baddr = bget(file->addrs[NDIRECT])->u32buf[blk_no];
...
void iappend(dinode_t *file, const void *buf, uint32_t size) {
  ...
  while ((int)size > 0) {
    blk_t *b = iwalk(file, file->size / BLK_SIZE);
    uint32_t boffset = file->size % BLK_SIZE;
    uint32_t wrsize = (size >= BLK_SIZE - boffset ? BLK_SIZE - boffset : size);
    memcpy(&b->u8buf[boffset], buffer, wrsize);
    size -= wrsize;
    file->size += wrsize;
    buffer += wrsize;
  }
...
void add_file(char *path) {
  ...
  while ((nread = fread(buf, sizeof(char), 1024, fp)) > 0) {
    iappend(inode, buf, nread * sizeof(char));
  }
  fclose(fp);
}
