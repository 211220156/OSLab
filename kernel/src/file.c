...
file_t *fopen(const char *path, int mode) {
  ...
  if (type == TYPE_FILE || type == TYPE_DIR) {
    ...
    if ((mode & O_DIR) == O_DIR && type != TYPE_DIR)  goto bad;
    // TODO: Lab3-2, if type is DIR, go bad if mode WRITE or TRUNC
    if (type == TYPE_DIR && ((mode & O_WRONLY) == O_WRONLY || (mode & O_RDWR) == O_RDWR || (mode & O_TRUNC) == O_TRUNC))  goto bad;
    // TODO: Lab3-2, if mode&O_TRUNC, trunc the file
    if (type == TYPE_FILE && (mode & O_TRUNC) == O_TRUNC)
      itrunc(ip);
...
int fread(file_t *file, void *buf, uint32_t size) {
  ...
  if (file->type == TYPE_FILE) {
    int num = iread(file->inode, file->offset, buf, size);
    if (num != -1)
      file->offset += num;
    return num;
  }
...
int fwrite(file_t *file, const void *buf, uint32_t size) {
  ...
  //TODO();
  if (file->type == TYPE_FILE) {
    int num = iwrite(file->inode, file->offset, buf, size);
    if (num != -1)
      file->offset += num;
    return num;
  }
