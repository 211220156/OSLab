...
static void bfree(uint32_t blkno) {
  ...
  byte = (((byte >> (blkno % 8)) & (0xff - 1)) << (blkno % 8)) | (byte & ((1 << (blkno % 8)) - 1));
...
static inode_t *ilookup(inode_t *parent, const char *name, uint32_t *off, int type) {
  ...
  if (type == TYPE_DIR)
    idirinit(ret, parent);
  dirent_t d;
  iread(parent, empty, &d, sizeof d);
  d.inode = no;
  strcpy(d.name, name);
  iwrite(parent, empty, &d, sizeof d);
  if (off != NULL)
    *off = empty;
  return ret;
}
...
//  printf("iwalk no : %d\n", no);
  ...
  if (no < NINDIRECT) {
    ...
    uint32_t ret;
    bread(&ret, 4, inode->dinode.addrs[NDIRECT], 4 * no);
    if (ret == 0) {
      ret = balloc();
      bwrite(&ret, 4, inode->dinode.addrs[NDIRECT], 4 * no);
    }
    return ret;
  }
...
int iread(inode_t *inode, uint32_t off, void *buf, uint32_t len) {
  ...
  //TODO();
  uint32_t no = off / BLK_SIZE, offset = off % BLK_SIZE;
  uint32_t blk_no = iwalk(inode, no);
  char *buffer = (char*)buf;
  int cnt = 0;
  bool reach_end = false;
  while ((int)len > 0 && !reach_end) {
    uint32_t read_size = (len <= BLK_SIZE - offset ? len : BLK_SIZE - offset);
    if (read_size + cnt + off >= inode->dinode.size) {
      reach_end = true;
      read_size = inode->dinode.size - cnt - off;
    }
    bread(buffer, read_size, blk_no, offset);
    len -= read_size;
    buffer += read_size;
    cnt += read_size;
    offset = 0;
    no += 1;
    blk_no = iwalk(inode, no);
  }
  return cnt;
}
...
int iwrite(inode_t *inode, uint32_t off, const void *buf, uint32_t len) {
  ...
  char *buffer = (char*)buf;
  while ((int)len > 0) {
    uint32_t write_size = (len <= BLK_SIZE - offset ? len : BLK_SIZE - offset);
    bwrite(buffer, write_size, blk_no, offset);
    len -= write_size;
    buffer += write_size;
    cnt += write_size;
    offset = 0;
...
void itrunc(inode_t *inode) {
  ...
  if (inode->dinode.addrs[NDIRECT] != 0) {
    ...
    for (int i = 0; i < NINDIRECT; i++) {
      bread(&blk_no, 4, inode->dinode.addrs[NDIRECT], i * 4);
      if (blk_no != 0) {
        bfree(blk_no);
        blk_no = 0;
        bwrite(&blk_no, 4, inode->dinode.addrs[NDIRECT], i * 4);
      }
    }
...
int iremove(const char *path) {
  ...
  uint32_t offset;
  inode_t *file = ilookup(parent, name, &offset, TYPE_NONE);
  if (file == NULL) {
    iclose(parent);
    return -1;
  }
  if (file->dinode.type == TYPE_DIR && !idirempty(file)) {
    iclose(parent);
    iclose(file);
    return -1;
  }
